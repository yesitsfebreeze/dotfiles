...
local function check_both_scrolloff(ev)
  if is_disabled() then
    return
  end
  ...
  -- Check top (BOF)
  local visual_distance_to_bof = win_cur_line - 1
  if visual_distance_to_bof < scrolloff then
    local adjustment = scrolloff - visual_distance_to_bof
    vim.fn.winrestview({
      skipcol = 0,
      topline = math.max(1, win_view.topline - adjustment),
    })
  end
end
...
local default_opts = {
  ...
  insert_mode = false,
  floating = true,
  disabled_filetypes = { 'terminal', 'oil' },
  disabled_modes = { 't', 'nt' },
}
...
M.setup = function(opts)
  ...
  local scrollBoth_group = vim.api.nvim_create_augroup('ScrollBoth', { clear = true })

  vim.api.nvim_create_autocmd('ModeChanged', {
    group = scrollBoth_group,
