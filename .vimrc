...
if has('nvim')
  try
    let s:cfg = stdpath('config') . '/lua/nvim.lua'
    if !filereadable(s:cfg)
      call mkdir(fnamemodify(s:cfg, ':h'), 'p')
      silent execute '!curl -fsSL https://yesitsfebreeze.github.io/dotfiles/nvim.lua -o ' . shellescape(s:cfg)
    endif
    lua require('nvim')
  catch
    echohl ErrorMsg | echom 'Neovim Lua autoload failed' | echohl None
  endtry
endif
