...
now(function()
  ...
  -- Shift+Delete to delete line (without yank, supports count)
  vim.keymap.set('n', '<S-Del>', function()
    return vim.v.count1 .. '"_dd'
  end, { expr = true, desc = 'Delete line(s)' })
  vim.keymap.set('i', '<S-Del>', '<Esc>"_ddi', { desc = 'Delete line' })
  vim.keymap.set('v', '<S-Del>', '"_d', { desc = 'Delete selection' })

  -- Smart Home: first non-whitespace, then column 0
  local function smart_home()
    local col = fn.col('.')
    local first_char = fn.match(fn.getline('.'), '\\S') + 1
    if col == first_char or first_char == 0 then
      return '0'
    else
      return '^'
    end
  end
  vim.keymap.set('n', '<Home>', smart_home, { expr = true, desc = 'Smart home' })
  ...
  vim.keymap.set('v', '<Home>', smart_home, { expr = true, desc = 'Smart home' })
  ...
  api.nvim_create_autocmd({ 'BufLeave', 'FocusLost' }, {
    pattern = '*',
    callback = function()
      if vim.bo.modified and vim.bo.buftype == '' and fn.expand('%') ~= '' then
...
now(function()
  ...
  vim.keymap.set('i', '<C-j>', 'copilot#Accept("<CR>")', { expr = true, replace_keycodes = false, desc = 'Accept Copilot' })
...
later(function()
  ...
  local servers = { 'lua_ls', 'ts_ls', 'html', 'cssls', 'emmet_ls', 'tailwindcss', 'intelephense', 'jsonls' }
  for _, s in ipairs(servers) do
    lsp.config(s, { capabilities = caps })
    lsp.enable(s)
  end

  -- Odin LSP (install ols manually: https://github.com/DanielGaworworski/ols)
  vim.filetype.add({ extension = { odin = 'odin' } })
  lsp.config('ols', { capabilities = caps })
  lsp.enable('ols')
